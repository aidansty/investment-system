...
def build_daily_briefing_prompt(regime, macro, news, candidates, positions, today):
    ...
    return (
        ...
        "---\n\n"
        "## Developing Candidates\n"
        "One bullet per candidate. One sentence only. No scores, no numbers, no methodology.\n"
        "Write the specific reason it is not Strong using plain English from the missing_signal field.\n"
        "Good: '- FTNT: Strong momentum but no earnings catalyst confirmed in the next 42 days.'\n"
        "Good: '- DDOG: Up 16% in the last 5 days — overextended, wait for a pullback.'\n"
        "Good: '- AMD: Already held as an open position — monitor existing trade.'\n"
        "Bad (never write): '- FTNT: Score of 0.630 indicates it is building toward a strong signal.'\n\n"
        "## Risk Assessment\n"
